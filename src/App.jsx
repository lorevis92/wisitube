...
export default function App() {
  ...
  async function handleResume(record) {
    ...
    // MERGE, not a selective rebuild. This used to be a hand-listed allow-list of project fields to
    // carry across the resume, and every feature that added a field to `project` (promiseFulfilled,
    // subject, staticBackground, the companion-Short markers…) had to remember to add it here too —
    // forgetting meant the field was silently wiped the first time this video's autosave rewrote its
    // jsonb column via plain saveVideo. So instead: take the WHOLE persisted project blob forward
    // and only override what the resume genuinely has to touch (rebuilt media, structural defaults).
    //
    // The split between "column" and "project blob" is exactly the one videoRowFrom (src/lib/db.js)
    // uses on write — mirror it here. These seven must NOT leak into `project` state:
    //   - settings: owned by its own `settings` state (setSettings below); a stale copy in `project`
    //     would let the autosave's `...project` clobber the live settings.
    //   - channelId: the autosave deliberately uses openVideoChannelId, not the video's own field
    //     (see the "silent channel reassignment" fix / setOpenVideoChannelId below) — a stale
    //     project.channelId winning that spread would reintroduce that bug.
    //   - updatedAt: videoRowFrom doesn't destructure it, so a copy in `project` becomes jsonb cruft
    //     that then shadows the real updated_at column on the next read.
    //   - id / createdAt / topic / displayTitle: plain columns, regenerated by the autosave.
    // promisedFollowUp / promiseFulfilled ARE kept in the blob on purpose — the autosave needs them
    // in `project` so videoRowFrom can write them back to their columns.
    const { id: _id, channelId: _channelId, createdAt: _createdAt, updatedAt: _updatedAt, topic: _topic, settings: _settings, displayTitle: _displayTitle, ...projectBlob } = record;
    ...
    // rehydrateProjectMedia already normalizes scenes / references / staticBackground /
    // renderedVideoBlob; the overrides below only cover the remaining structural containers that
    // downstream code dereferences without a guard (project.titles[…], project.thumbnails[0],
    // resumedProject.pendingImageBatches.length just after this call).
    let resumedProject = await rehydrateProjectMedia({
      ...projectBlob,
      titles: projectBlob.titles || [],
      selectedTitle: projectBlob.selectedTitle || 0,
      tags: projectBlob.tags || [],
      thumbnails: projectBlob.thumbnails || [],
      references: projectBlob.references || [],
      characterBible: projectBlob.characterBible || [],
      scenes: projectBlob.scenes || [],
      pendingImageBatches: Array.isArray(projectBlob.pendingImageBatches) ? projectBlob.pendingImageBatches : [],
      batchRecoveryCycles: Number(projectBlob.batchRecoveryCycles) || 0,
      // Never persisted (the final MP4 lives only in memory) — a genuine resume always re-renders.
