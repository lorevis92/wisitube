...
JSON schema:
{
  ...
  "thumbnail_concepts": [3 objects: { "overlay_text": "punchy text, max 4 words UPPERCASE unless the channel's thumbnail creative direction says otherwise — the ONLY text that appears on the thumbnail, so it must say one thing clearly", "image_prompt": "concrete visual description in English for an AI image generator — follows this channel's own thumbnail creative direction (Thumbnail settings, per video/Short) exactly when one is set; otherwise defaults to one strong focal subject, exaggerated emotion, no text in image. If a real, identifiable person or a well-known named character is central to this video, that focal subject MUST be named explicitly by their proper name (e.g. \"Elon Musk with a shocked expression, plunging red stock-market graphs behind him\") — never a generic stand-in like \"a businessman\" or \"a shocked man\"" }],
...
- thumbnail_concepts: when this video centers on a real, identifiable person or a well-known named character (the same figures listed in character_bible), every concept's image_prompt MUST name that subject explicitly by their proper name — the same principle as the title naming its real subject, not a generic lookalike description. Only fall back to a generic figure when the video genuinely has no single identifiable person or character at its center.
